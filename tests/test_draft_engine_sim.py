...
tracker and asks the engine for a record at every sale — the seam the
backtest (issue #6) will use — and must produce byte-identical records
...
def _replay_once(config, rows):
    ...
    while board.status != "complete":
        ...
        records.append(analyze_player(nominee, rows, board, config))
    return records
...
def test_engine_prices_every_2025_sale_deterministically(config):
    ...
    record at all 180 sale moments (my slot resolving from the config's
    roster id through the 2025 slot map), every bid is a whole-dollar
    number within my cap, the off-model lots price at the floor, the
    board's repricing genuinely moves mid-draft, and two full replays
    are byte-identical."""
    data = json.loads(FIXTURE.read_text(encoding="utf-8"))
